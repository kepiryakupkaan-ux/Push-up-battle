 ...
 *
 * ÖNEMLİ DÜZELTME (v2): Bu sınıf PeerConnection'ı UNIFIED_PLAN modunda kuruyor.
 * Unified Plan'da karşı taraftan gelen track'ler `onAddStream` ile DEĞİL,
 * `onAddTrack` ile bildirilir - eski kod sadece onAddStream dinlediği için
 * bağlantı "connected" olsa bile karşı tarafın görüntüsü hiçbir zaman
 * gelmiyordu (sadece kendi kameranı görüyordun). Şimdi onAddTrack asıl yol,
 * onAddStream ise yedek/garanti olarak duruyor.
 */
class WebRtcClient(
    ...
    fun createPeerConnection(iceServers: List<PeerConnection.IceServer>) {
        val rtcConfig = PeerConnection.RTCConfiguration(iceServers).apply {
            ...
            // TCP candidate'ları da dene - bazı ağlarda (okul/kurum wifi'si gibi UDP'yi
            // kısıtlayan ağlarda) bağlantının hiç kurulamamasını önler.
        ...
        peerConnection = peerConnectionFactory.createPeerConnection(
            ...
            object : PeerConnection.Observer {
                ...
                // Unified Plan'da asıl bildirim yolu budur.
                ...
                // Legacy/Plan B yolu - bazı sürümlerde ek güvence olarak burada da yakalıyoruz.
        ...
        // Track'leri ortak bir stream id'siyle ekliyoruz; bu, karşı taraftaki
        // onAddStream/onAddTrack'in video+ses'i aynı stream'in parçası olarak
        // görmesini sağlar (bazı istemcilerde stream id boşsa track eşleşmesi bozulabiliyor).
    ...
    /** Karşı taraftan gelen video track'ini verilen renderer'a bağlar. */
