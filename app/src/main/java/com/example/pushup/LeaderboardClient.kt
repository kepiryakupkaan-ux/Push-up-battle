    ...
    fun fetchTopPlayers(mode: LeaderboardMode, limit: Long = 50, onResult: (List<LeaderboardEntry>) -> Unit) {
        ...
        query.get()
            ...
            .addOnFailureListener { e ->
                // WEEKLY modu weekStartTimestamp (eşitlik) + weeklyPoints (sıralama) birlikte
                // kullandığı için composite index gerektiriyor - eksikse burada patlar ve
                // "Bu Hafta" sekmesi sessizce bomboş görünürdü. Artık en azından loglanıyor.
                Log.e("PushUpLeaderboard", "fetchTopPlayers($mode) failed - index eksik olabilir", e)
                onResult(emptyList())
            }
    }
