...
class PoseAnalyzer(
     ...
     *
     * DÜZELTME: 90 ve 270 derece durumları daha önce birbiriyle yer değiştirmişti - bu
     * yüzden iskelet eğik/çapraz görünüyordu. Saat yönünde döndürme formülleri baştan
     * türetilip doğrulandı (köşe noktalarıyla test edildi), aşağıdaki eşleme doğrusu.
     */
    private fun rotatedMirroredNormalized(x: Float, y: Float, imgW: Int, imgH: Int, rotationDeg: Int): PointF {
        val nx: Float
        val ny: Float
        when (rotationDeg) {
            90 -> { nx = 1f - y / imgH; ny = x / imgW }
            180 -> { nx = 1f - x / imgW; ny = 1f - y / imgH }
            270 -> { nx = y / imgH; ny = 1f - x / imgW }
