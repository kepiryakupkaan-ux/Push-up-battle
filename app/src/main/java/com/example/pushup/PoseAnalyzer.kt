...
class PoseAnalyzer(
     ...
     *
     * DÜZELTME (4. tur - kullanıcı geri bildirimi): iskelet, kamera penceresinin SOL
     * tarafında, sağa yatık duruyordu; yani bir önceki turda uygulanan düzeltme yetersizdi.
     * Kullanıcının tarif ettiği kaymayı gidermek için önceki sonucun üstüne BİR 90° SAAT
     * YÖNÜNDE döndürme daha uygulandı (toplamda iki kez 90° CW = eskiye göre 180°'lik fark).
     * Rotate-90°-CW formülü birim karede: (x,y) -> (1-y, x).
     *   1) sensör döndürmesine göre ham -> (nx, ny) [0..1]
     *   2) selfie aynalama: (mx, my) = (1-nx, ny)
     *   3) eski "düzeltme": (1-my, mx)
     *   4) yeni ekstra 90° CW: (1-eskiY, eskiX) -> sadeleşince (nx, 1-ny)
     * Yani nihai dönüşüm: (nx, 1 - ny).
     */
